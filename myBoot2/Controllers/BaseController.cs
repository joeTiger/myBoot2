...
namespace myBoot2.Controllers
{
    public class BaseController : Controller
    {
        ...
        public void setViewBagCatPdtModel(int bizId, int lg, int catId, int subId)
        {
            ...
            {
                mylog(sPdt + " is NULL !!!!!!!!!!!!!!!");
                IEnumerable<spGetGridPdtByBizResult> list = null;
                //extract cat from Pdts
                //if subId=0 get cat and subCats    => Deserts, subDes1, subDes2
                //else get only cat with subCatId   => Deserts, subDes1 if subCatId=subDes1
                if (subId == 0)
                {
                    list = getListPdtCatByLg(bizId, lg, catId);
                }
                else
                {
                    ...
                    {
                        list = (IEnumerable<spGetGridPdtByBizResult>)Session[s];
                    }
                }
                List<spGetGridPdtByBizResult> listCat = getListCat(subId, list);
                if (subId > 0) list = list.Where(x => x.catId == subId).ToList();
                CheckListGridPdtByBiz(sCat, listCat);
                CheckListGridPdtByBiz(sPdt, list);
            ...
            {
                mylog(sCat + " is NULL !!!!!!!!!!!!!!!");
                List<spGetGridPdtByBizResult> listCat = getListCat(subId, (IEnumerable<spGetGridPdtByBizResult>)Session[sPdt]);
                Session[sCat] = listCat;
            }
            if (Session[sPdt] == null)
            {
        ...
        private static List<spGetGridPdtByBizResult> getListCat(int subId, IEnumerable<spGetGridPdtByBizResult> list)
        {
        ...
        private IEnumerable<spGetGridPdtByBizResult> getListPdtCatByLg(int bizId, int lg, int catId)
        {
        ...
        private void writeToLogfile(string s)
        {
            try
            {
                FileStream fs = new FileStream("C:\\log\\logFileBoot.txt", FileMode.Append, FileAccess.Write);
